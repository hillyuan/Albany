...
#ifndef ALBANY_THERMOCONDUCTPROBLEM_HPP
#define ALBANY_THERMOCONDUCTPROBLEM_HPP
...
namespace Albany {
  ...
  private:
    ...
    ThermoConductProblem(const ThermoConductProblem&);

    //! Private to prohibit copying
    ThermoConductProblem& operator=(const ThermoConductProblem&);
...
template <typename EvalT>
Teuchos::RCP<const PHX::FieldTag>
Albany::ThermoConductProblem::constructEvaluators(
  PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
  const Albany::MeshSpecsStruct& meshSpecs,
  Albany::StateManager& stateMgr,
  ...
  const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      std::logic_error,
      "Albany_ThermoConductProblem must be defined as a steady or transient calculation.");
...
#endif // ALBANY_THERMOCONDUCTPROBLEM_HPP
