  ...
  coordVec       (p.get<std::string>("Coordinate Vector Name"), dl->vertices_vector)
{
  ...
  name = p.get< std::string >("BodySource Input String");

  // The input.xml argument for the above string
  inputValues = p.get<Teuchos::Array<double> >("BodySource Input Value");

  // The input.xml argument for the above string
  inputConditions = p.get< std::string >("BodySource Input Conditions");
  ...
  // If we are doing a BodySource internal boundary with a "scaled jump",
  ...
  if((inputConditions == "scaled jump" || inputConditions == "robin" || inputConditions == "radiate") &&
    ...
    if(inputConditions == "scaled jump") {
      ...
      robin_vals[2] = inputValues[2]; // jump in slope (like plain BodySource bc)
...
//        << it->first << std::endl;
     ...
     if (inputConditions == "robin" || inputConditions == "radiate") {
      // Currently, the BodySource evaluator doesn't handle the case when the degree of freedom is a vector.
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(p.get<bool>("Vector Field") == true,
                             ...
                             std::endl << "Error: \"Robin\" BodySource boundary conditions "
...
//**********************************************************************
template<typename EvalT, typename Traits>
void BodySourceBase<EvalT, Traits>::
...
template<typename EvalT, typename Traits>
void BodySourceBase<EvalT, Traits>::
evaluateBodySourceContribution(typename Traits::EvalData workset)
{
...
template<typename EvalT, typename Traits>
typename BodySourceBase<EvalT, Traits>::ScalarT&
BodySourceBase<EvalT, Traits>::
...
/*
  double kdTdx[3];
  kdTdx[0] = 1.0; // BodySource component in the x direction
  kdTdx[1] = 0.0; // BodySource component in the y direction
  kdTdx[2] = 0.0; // BodySource component in the z direction
*/
    ...
    for(int pt = 0; pt < numPoints; pt++)
      for(int dim = 0; dim < numDOFsSet; dim++)
        qp_data_returned(cell, pt, dim) = coeff*(dof_side(cell,pt) - dof_value) - jump * scale * 2.0;
         ...
         //  in which case usual BodySource would add contributions from both sides, giving factor of 2
}
...
calc_dudn_basal(Kokkos::DynRankView<ScalarT, PHX::Device> & qp_data_returned,
 ...
 //Robin/BodySource bc for FELIX FO XZ MMS test case
...
BodySource<PHAL::AlbanyTraits::Residual,Traits>::
BodySource(Teuchos::ParameterList& p)
  : BodySourceBase<PHAL::AlbanyTraits::Residual,Traits>(p)
{
}
...
template<typename Traits>
void BodySource<PHAL::AlbanyTraits::Residual, Traits>::
evaluateFields(typename Traits::EvalData workset)
{
  ...
  this->evaluateBodySourceContribution(workset);
...
template<typename Traits>
BodySource<PHAL::AlbanyTraits::Jacobian, Traits>::
BodySource(Teuchos::ParameterList& p)
  : BodySourceBase<PHAL::AlbanyTraits::Jacobian,Traits>(p)
{
}
// **********************************************************************
...
void BodySource<PHAL::AlbanyTraits::Jacobian,Traits>::
operator()(const BodySource_Tag& tag, const int& cell) const
{
...
//    JacT->resumeFill();
   ...
   Kokkos::parallel_for(BodySource_Policy(0,workset.numCells),*this);
...
template<typename Traits>
BodySource<PHAL::AlbanyTraits::Tangent, Traits>::
BodySource(Teuchos::ParameterList& p)
  : BodySourceBase<PHAL::AlbanyTraits::Tangent,Traits>(p)
{
}

// **********************************************************************
template<typename Traits>
void BodySource<PHAL::AlbanyTraits::Tangent, Traits>::
evaluateFields(typename Traits::EvalData workset)
{
  auto nodeID = workset.wsElNodeEqID;
  Teuchos::RCP<Tpetra_Vector> fT = workset.fT;
...
template<typename Traits>
BodySource<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
BodySource(Teuchos::ParameterList& p)
  : BodySourceBase<PHAL::AlbanyTraits::DistParamDeriv,Traits>(p)
{
}

// **********************************************************************
template<typename Traits>
void BodySource<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
evaluateFields(typename Traits::EvalData workset)
{
  auto nodeID = workset.wsElNodeEqID;
...
// **********************************************************************
// Simple evaluator to aggregate all BodySource BCs into one "field"
// **********************************************************************

template<typename EvalT, typename Traits>
BodySourceAggregator<EvalT, Traits>::
BodySourceAggregator(const Teuchos::ParameterList& p)
{
  ...
  this->setName("BodySource Aggregator"+PHX::typeAsString<EvalT>());
}
