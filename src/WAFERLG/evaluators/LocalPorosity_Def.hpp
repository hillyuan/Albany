...
namespace WAFERLG {
  ...
  LocalPorosity<EvalT, Traits>::
  LocalPorosity(Teuchos::ParameterList& p, const Teuchos::RCP<Albany::Layouts>& dl) :
...

  {
    ...
    Teuchos::RCP<const Teuchos::ParameterList> reflist =
      this->getValidLocalPorosityParameters();
  ...
  //**********************************************************************
  template<typename EvalT, typename Traits>
  void LocalPorosity<EvalT, Traits>::
  ...
  evaluateFields(typename Traits::EvalData workset)
  {
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell)
	  ...
	  {
            porosity_(cell, qp) = Initial_porosity*(1.0 - psi1_(cell, qp));
	  }
      }
  }
