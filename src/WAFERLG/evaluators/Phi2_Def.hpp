...
namespace WAFERLG {
    ...
    phi2_        (p.get<std::string>("Phi2 Name"),dl->qp_scalar)
  {
    ...
    type = cond_list->get("Vapourization Temperature Type", "Constant");
    VapourizationTemperature_ = cond_list->get("Vapourization Temperature Value", 3300.0);

    type = cond_list->get("delta Temperature Type", "Constant");
    deltaTemperature_ = cond_list->get("delta Temperature Value", 50.0); 

    this->setName("Phi2"+PHX::typeAsString<EvalT>());
  }
  ...
  evaluateFields(typename Traits::EvalData workset)
  {
    ...
    if (t > 0.0)
        ...
        for (std::size_t cell = 0; cell < workset.numCells; ++cell)
	  {
	      ...
	      {
                phi2_(cell, qp) = 0.5 * (std::tanh((T_(cell, qp) - VapourizationTemperature_) / deltaTemperature_) + 1.0);
	      }
	  }
      }
  }
  ...
  getValidPhi2Parameters() const
  {
    ...
    valid_pl->set<std::string>("Vapourization Temperature Type", "Constant");
    valid_pl->set<double>("Vapourization Temperature Value", 1.0);
